...
export default function DRAW({ user, Draw, mainindex }) {
    let pkr = Intl.NumberFormat('en-PK', { style: 'currency', currency: 'PKR' });
    ...
    useEffect(() => {
        let side = document.getElementById(Draw.drawType + "Drawside" + mainindex);
        let leftside = document.getElementById(Draw.drawType + "Drawleftside" + mainindex);
        let interval = setInterval(() => {
            if (side && leftside && document.getElementById('pageloading').classList.contains('hidden')) {
                side.classList.add('animate-[slideLeftFadeIn_1s_ease]');
                leftside.classList.add('animate-[slideRightFadeIn_1s_ease]');
                clearInterval(interval);
            }
        }, 200);
    }, [Draw.drawType, Draw.drawName, mainindex])

    return (
        <div key={mainindex} id={Draw.drawType + "Draw" + mainindex}>
            <div className="relative grid grid-cols-1 md:grid-cols-2 my-12 gap-10">
                <div id={Draw.drawType + "Drawleftside" + mainindex} style={{ width: "100%" }} className="relative rounded-lg overflow-hidden mx-auto pb-4">
                ...
                <div id={Draw.drawType + "Drawside" + mainindex} className="relative flex flex-col justify-center" suppressHydrationWarning>
                    <section className="flex flex-col gap-3">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="bg-gray-800 rounded-full py-1 px-3 text-sm text-center text-white">{!parseInt(Draw.prizes[0]) ? Draw.prizes[0] : pkr.format(Draw.prizes[0] / 100)}</div>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="bg-gray-800 rounded-full py-1 px-3 text-sm text-center text-white">{!parseInt(Draw.prizes[1]) ? Draw.prizes[1] : pkr.format(Draw.prizes[1] / 100)}</div>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="bg-gray-800 rounded-full py-1 px-3 text-sm text-center text-white">{!parseInt(Draw.prizes[2]) ? Draw.prizes[2] : pkr.format(Draw.prizes[2] / 100)}</div>
                        </div>
                    </section>
